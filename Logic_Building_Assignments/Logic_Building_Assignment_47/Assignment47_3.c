...
int CountWhiteSpaces(char FName[])
{
    ...
    while((iByte = read(fd,Buffer,sizeof(Buffer))) != 0)        //Read the data into local Bufferay
    {
        for(i = 0; i < iByte; i++)
        {
            if(Buffer[i] == ' ')
            {
                iCount++;                                       //Count WhiteSpaces characters
            }
        }
    }
...
/*
    ...
    Output:
    D:\Marvellous_Infosystems\Logic_Building_with_Industrial_Project_Batch\Logic_Building_Assignments\Logic_Building_Assignment_47>gcc Assignment47_3.c 
